...
//	instances is a Forwarding server address lists
func proxyingMiddleware(ctx context.Context, instances string, logger log.Logger) ServiceMiddleware {
	...
	var (
		...
		maxAttempts = 3                      // if three request are fail , give up              // per request, before giving up
	...
	// discovery system.
	var (
		instanceList = split(instances) //split url by ','
	...
	for _, instance := range instanceList {
		...
		// add a circuitbreakerMiddleware
		e = circuitbreaker.Gobreaker(gobreaker.NewCircuitBreaker(gobreaker.Settings{}))(e)
		// add a ratelimitMiddleware
